...
const systemFunc = {
  ...
  restart: function () {
    const firstflag = systemFunc.firstflag;
    if (firstflag) {
      console.info('%cScanning', 'font-weight: bold;');
      systemFunc.firstflag = false;
    } else {
...
const checkObjects = {
  ...
  mixTrendWord: function (usrId) {
    let t = 0;
    let boo = 0;
    const trendWords = trendObjects.trendWords;
    const tweetText = replyObjects.tweetTextList[usrId];
    if (trendWords) {
      tweetText.forEach(text => {
  ...
  tweetEval: function (replyEl, usrId, num) {
    ...
    if (observeObject.category === 'reply') {
      ...
      systemFunc.idPointList[num].trendWord = checkObjects.mixTrendWord(usrId);
    }
